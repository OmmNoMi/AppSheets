...
| Employee | Employee | Operational (Core) | Central employee profile (**Extended** Phase 3: InternshipID, UniversityID, EmploymentType, IsIntern VC) |
| EmployeeDocument | EmployeeDocument | Operational | Visas, passports, contracts (**Extended** Phase 3: Type enum + Internship Agreement, NDA, NOC, CoE, Certificate) |
| Project | Project | Operational | Master list of project bubbles |
| TaskList | TaskList | Operational | Onboarding/offboarding task tracker (**Extended** Phase 3: Category enum + intern categories) |
| CandidateSubmission | CandidateSubmission | Operational | Google Form staging area (**Extended** Phase B: InternApplication columns) |
...
| **Internship** | **Internship** | **Operational (Phase 3A)** | **Core intern lifecycle record — one per intern engagement** |
| **ComplianceRule** | **ComplianceRule** | **Operational (Phase 3A)** | **Config: jurisdiction compliance rules (pre-seeded from SOW Appendix B)** |
| **ComplianceItem** | **ComplianceItem** | **Operational (Phase 3A)** | **Per-intern compliance tracking items (auto-generated by bot)** |
| **LearningRecord** | **LearningRecord** | **Operational (Phase 3A)** | **Weekly EPIC learning log entries** |
| **MonthlyCheckIn** | **MonthlyCheckIn** | **Operational (Phase 3A)** | **Monthly manager-intern check-in records** |
| **EpicMilestone** | **EpicMilestone** | **Operational (Phase 3A)** | **Forward-planned EPIC milestones by month** |
| **MentorAssignment** | **MentorAssignment** | **Operational (Phase 3A)** | **Mentor/supervisor assignment records per internship** |
| **InternReview** | **InternReview** | **Operational (Phase 3A)** | **Regular, Mid and End reviews on Orbit 1-5 scale** |
| **InternFeedback** | **InternFeedback** | **Operational (Phase 3A)** | **Standardised intern satisfaction feedback (one per internship)** |
| **Cohort** | **Cohort** | **Operational (Phase 3B)** | **Intake cohort groupings** |
| **University** | **University** | **Operational (Phase 3B)** | **Consolidated university/institution register** |
| **Certificate** | **Certificate** | **Operational (Phase 3B)** | **Completion certificate issuance log** |
...
## Phase 3 — Early Careers Module (Added 18 June 2026)
> Full detail in `_AppDoc/EarlyCareer_Schema.md`, `_AppDoc/EarlyCareer_Automations.md`, `_AppDoc/EarlyCareer_UX_Views.md`

### New AppVariables (Roles)
| ID | Title | Notes |
|----|-------|-------|
| `U_People_Admin` | People Admin (HR) | Full intern lifecycle access |
| `U_Intern` | Intern | Self-service access to own records |
| `U_University_Coordinator` | University Coordinator | Read-only sponsored intern view |
| `InternStatus_List` | Intern Status Values | VariableList driving Internship.Status enum |
| `EPIC_List` | EPIC Component Values | VariableList driving LearningRecord.EpicComponent |

### New AppSettings
| ID | Title | Decimal | Notes |
|----|-------|---------|-------|
| `PassportMinValidityMonths` | Passport Minimum Validity | 7 | Months beyond internship end date |
| `InternWeeklyReminderDay` | Learning Reminder Day | 1 | 1 = Monday |
| `InternFeedbackDaysBeforeEnd` | Feedback Dispatch Lead Time | 14 | Days before end date |
| `ReviewCadenceDays` | Regular Review Cadence | 7 | 7 = weekly, 14 = biweekly |

### New Employee Columns (Extended)
| Column | Type | Notes |
|--------|------|-------|
| `EmploymentType` | Enum | Add `Intern` value to existing type enum |
| `InternshipID` | Enum Ref → Internship | Show_If: EmploymentType = Intern |
| `UniversityID` | Enum Ref → University | Phase B |
| `UniversityProgramme` | Text | Show_If: Intern |
| `AcademicSupervisor` | Text | Show_If: Intern |
| `ExpectedGraduation` | Date | Show_If: Intern |
| `IsIntern` (VC) | Virtual | `[EmploymentType] = "Intern"` |
| `InternshipStatus` (VC) | Virtual | `[InternshipID].[Status]` |
| `InternReviewCount` (VC) | Virtual | COUNT of related InternReview |

### New Enum Values Added to Existing Tables
| Table.Column | New Values Added |
|-------------|-----------------|
| `EmployeeDocument.Type` | Internship Agreement, Internship NDA, NOC (No Objection Certificate), Certificate of Eligibility, Completion Certificate, Internship Summary Report |
| `TaskList.Category` | Internship Onboarding - HR, Internship Onboarding - Line Manager, Internship Onboarding - IT, Internship Onboarding - Accounts, Internship Onboarding - PRO, Creative Futures - EPIC, Intern Review - Mid, Intern Review - End |

### New Tables Summary (Phase 3)
| Table | Phase | Rows Created By | Parent |
|-------|-------|-----------------|--------|
| `Internship` | A | HR manually / Bot | Employee |
| `ComplianceRule` | A | Pre-seeded config | None |
| `ComplianceItem` | A | Bot_ComplianceBuilder | Internship |
| `LearningRecord` | A | Intern (self) | Internship |
| `MonthlyCheckIn` | A | Line Manager | Internship |
| `EpicMilestone` | A | HR / Line Manager | Internship |
| `MentorAssignment` | A | HR | Internship |
| `InternReview` | A | Line Manager + Bot trigger | Internship |
| `InternFeedback` | A | Bot pre-creates, Intern fills | Internship |
| `Cohort` | B | HR | None |
| `University` | B | HR | None |
| `Certificate` | B | Bot_CertificateGenerator | Internship |

### Phase 3 Bot Registry
| Bot Name | Trigger | Phase |
|----------|---------|-------|
| `Bot_InternOnboardingGenerator` | Status → Onboarding (UPDATES) | A |
| `Bot_AgreementNDAGenerator` | Status → Agreement Sent (UPDATES) | A |
| `Bot_ComplianceBuilder` | Country set + 0 items exist (UPDATES) | A |
| `Bot_PassportValidityCheck` | EmployeeDocument (Passport) ADDS_AND_UPDATES | A |
| `Bot_WeeklyLearningReminder` | Scheduled: Every Monday 08:00 GST | A |
| `Bot_ReviewCadenceReminder` | Scheduled: Every Monday 08:00 GST | A |
| `Bot_MidReviewTrigger` | Scheduled: Daily 08:00 GST | A |
| `Bot_EndReviewTrigger` | Scheduled: Daily 08:00 GST | A |
| `Bot_FeedbackDispatch` | Scheduled: Daily 08:00 GST | A |
| `Bot_CertificateGenerator` | Status → Completed (UPDATES) | B |
| `Bot_ConversionHandoff` | Status → Converted (UPDATES) | B |
| `Bot_InternApplicationNotify` | ADDS_ONLY on CandidateSubmission | B |
