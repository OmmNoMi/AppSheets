// =========================================================================
// OmmNoMi Perfect Survey Engine: Enum/EnumList BaseType Ref -> AppVariables
// Protocol: Synchronized Redux Type, TypeAuxData, Valid_If, and IsLabel
// =========================================================================
(function configurePerfectOmmNoMiRefs() {
    console.log("🚀 Starting Perfect OmmNoMi Enum/EnumList Ref Configuration...");
    ...
    // 2. Configure AppVariables: ID is Key, Label is Label with trilingual formula
    if (appVarSchemaIdx !== -1) {
        ...
        avAttrs.forEach((attr, aIdx) => {
            ...
            if (attr.Name === 'ID') {
                ...
                console.log("🔹 AppVariables.Label configured with IsLabel=true & trilingual formula.");
            } else {
                if (attr.IsLabel) {
                    nameValueDict[`${p}.IsLabel`] = false;
                }
    ...
    // 3. TypeAuxData Templates for AppSheet Editor
    const refQualifier = JSON.stringify({
    ...
    const enumListTypeAux = JSON.stringify({
        ElementType: "Ref",
        ElementTypeQualifier: refQualifier,
        ItemSeparator: " , "
    });

    const enumTypeAux = JSON.stringify({
        EnumValues: [],
    ...
    surveyAttrs.forEach((attr, idx) => {
        ...
        if (colName && QMAP[colName]) {
            ...
            // Clear static EnumValues & SuggestedValues
            nameValueDict[`${path}.EnumValues`] = [];
            nameValueDict[`${path}.SuggestedValues`] = null;

            if (isMulti) {
                // Multi-select: EnumList with BaseType Ref -> AppVariables
                ...
                nameValueDict[`${path}.TypeAuxData`] = enumListTypeAux;
            } else {
                // Single-select: Enum with BaseType Ref -> AppVariables
                nameValueDict[`${path}.Type`] = 'Enum';
                nameValueDict[`${path}.EnumListElementTypeName`] = 'Ref';
                nameValueDict[`${path}.ReferencedTableName`] = 'AppVariables';
                nameValueDict[`${path}.TypeAuxData`] = enumTypeAux;
            }

            // OmmNoMi Valid_If Split Lookup
            ...
            if (attr.MetaData) {
                ...
                nameValueDict[`${path}.MetaData.ReferencedTableName`] = 'AppVariables';
            }
    ...
    console.log(`🎯 Successfully configured ${updatedCount} columns to Enum/EnumList BaseType Ref -> AppVariables!`);
    ...
    console.log("✅ Perfect OmmNoMi Architecture Applied!");
