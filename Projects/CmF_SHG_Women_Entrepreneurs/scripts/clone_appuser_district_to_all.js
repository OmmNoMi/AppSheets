// =========================================================================
// OmmNoMi: Clone AppUser.District Architecture to All Survey Dropdowns
// =========================================================================
(function cloneDistrictToAllSurvey() {
    console.clear();
    console.log("%c🚀 [OmmNoMi] Inspecting AppUser.District & Cloning to All Survey Dropdowns...", "color:#4285f4;font-size:15px;font-weight:bold;");
    ...
    if (openDoneBtn) {
        console.log("⚠️ Closing open column dialog first...");
        openDoneBtn.click();
    }
    ...
    if (appUserSchemaIdx === -1) {
        console.error("❌ AppUser schema not found!");
        return;
    }
    ...
    if (!districtAttr) {
        console.error("❌ District column not found in AppUser table!");
        return;
    }

    console.log("%c📌 Found AppUser.District Reference Column:", "color:#34a853;font-weight:bold;");
    console.log({
        Name: districtAttr.Name,
        Type: districtAttr.Type,
        BaseType: districtAttr.BaseType,
        ReferencedTableName: districtAttr.ReferencedTableName,
        ReferencedRootTableName: districtAttr.ReferencedRootTableName,
        EnumListElementTypeName: districtAttr.EnumListElementTypeName,
        ValidIf: districtAttr.ValidIf || districtAttr.Valid_If,
        TypeAuxData_Raw: districtAttr.TypeAuxData
    });

    let districtAux = {};
    try {
        districtAux = JSON.parse(districtAttr.TypeAuxData || "{}");
        console.log("📌 Parsed AppUser.District TypeAuxData:", districtAux);
    } catch(e) {
        console.warn("Could not parse district TypeAuxData:", e);
    }

    // Extract BaseTypeQualifier or ElementTypeQualifier template
    let baseQualifierStr = districtAux.BaseTypeQualifier || districtAux.ElementTypeQualifier;
    if (!baseQualifierStr) {
        // Fallback standard qualifier
        ...
        // If it was referencing another table, ensure it points to AppVariables
        try {
    ...
    // 3. Ensure AppVariables has Label as Label & ID as Key
    if (appVarSchemaIdx !== -1) {
        ...
        console.log("🔹 AppVariables schema synced: ID=Key, Label=Label (Multilingual formula).");
    }

    // 4. Mapped 83 Survey Questions
    const QMAP = {
    ...
    surveyAttrs.forEach((attr, idx) => {
        ...
        if (colName && QMAP[colName]) {
            ...
            if (isMulti) {
                ...
                // Single-select Enum with BaseType = Ref to AppVariables (Identical to AppUser.District)
                typeAuxObj = {
                    EnumValues: [],
                    ...
                    EnumInputMode: districtAux.EnumInputMode || "Auto",
                    Valid_If: validIfFormula,
                    Error_Message_If_Invalid: null,
                    Show_If: null,
                    Required_If: null,
    ...
    console.log(`🎯 Successfully cloned AppUser.District configuration to ${updatedCount} Survey columns!`);
    ...
    // 6. Trigger calculation & Light up SAVE button
    try {
    ...
    // 7. Verify result directly from state
    ...
    console.log(`%c🎉 CLONE SUCCESS! All ${updatedCount} Survey Questions now match AppUser.District architecture!`, "color:#34a853;font-size:15px;font-weight:bold;");
    console.log("📋 Verified Block (Single-select):", {
        Type: postBlock?.Type,
        BaseType: postBlock?.BaseType,
        RefTable: postBlock?.ReferencedTableName,
        ValidIf: postBlock?.Valid_If || postBlock?.ValidIf
    });
    console.log("📋 Verified BusinessType (Multi-select):", {
