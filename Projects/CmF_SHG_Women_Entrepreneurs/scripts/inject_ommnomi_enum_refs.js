// =========================================================================
// OmmNoMi Survey Engine: OmmNoMi Style Enum Ref + SPLIT LOOKUP Injector
// =========================================================================
(function injectOmmNoMiEnumRefs() {
    console.log("🚀 Starting OmmNoMi Style Enum Ref + SPLIT LOOKUP Injection...");
    ...
    // 2. Configure AppVariables Table: ID is Key, Label is Label with trilingual formula
    if (appVarSchemaIdx !== -1) {
        const avAttrs = schemas[appVarSchemaIdx].Attributes;
        
        const idIdx = avAttrs.findIndex(a => a.Name === 'ID');
        if (idIdx !== -1) {
            nameValueDict[`AppData.DataSchemas[${appVarSchemaIdx}].Attributes[${idIdx}].IsKey`] = true;
            console.log("🔹 AppVariables.ID set as Key.");
        }
        ...
        if (labelIdx !== -1) {
            ...
            console.log("🔹 AppVariables.Label configured with trilingual formula and IsLabel=true.");
        }
    }

    // 3. Mapped Survey Questions:
    // OmmNoMi Standard:
    // Single-Select: Type = 'Enum', BaseType = 'Ref', ReferencedTableName = 'AppVariables'
    // Multi-Select:  Type = 'EnumList', EnumListElementTypeName = 'Ref', ReferencedTableName = 'AppVariables'
    // ValidIf = '=SPLIT(LOOKUP("<QuestionID>", "AppVariables", "ID", "VariableList"), " , ")'
    const QMAP = {
    "Status_Profile": {
        "col": "Status_Profile",
        "qid": "Q_STAT_PROFILE",
    ...
    surveyAttrs.forEach((attr, idx) => {
        ...
        if (colName && QMAP[colName]) {
            ...
            // Reset any previous raw SuggestedValues so ValidIf takes full control
            nameValueDict[`${path}.SuggestedValues`] = null;

            if (isMulti) {
                // Multi-select: EnumList with Ref
    ...
    console.log(`🎯 Successfully configured ${updatedCount} Survey columns to OmmNoMi Enum Ref + SPLIT LOOKUP!`);
    console.log(`📦 Total Redux updates queued: ${Object.keys(nameValueDict).length}`);

    // 4. Dispatch batch update to AppSheet Redux store
    store.dispatch({
    ...
    // 5. Trigger calculation & Light up the SAVE button
    try {
    ...
    console.log("✅ OmmNoMi Survey Engine Ref & Dropdown Configuration Complete!");
    console.log("👉 Click the blue 'SAVE' button in AppSheet Editor now!");
})();
